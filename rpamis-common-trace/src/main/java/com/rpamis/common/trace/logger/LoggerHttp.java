...
public class LoggerHttp {
	...
	public static void update(@NotNull RequestLog requestLog, TraceResponseWrapper traceResponseWrapper, long totalTime)
			throws IOException {
		...
		if (contentType.isPresent() && "application/json".equals(contentType.get())) {
			requestLog.setResponse(JackSonUtil.toMap(new String(traceResponseWrapper.getContentAsByteArray())));
		}
