...
public class EnumCacheBuilder {
	...
	private static void cacheEnums() {
		if (scanPackages == null || scanPackages.isEmpty()) {
			...
			List<Class<? extends CachableEnum<?, ?>>> implClasses = ClassScanner
				.getCachableEnumImplClassesByPackages(scanPackages.toArray(new String[0]));
			for (Class<? extends CachableEnum<?, ?>> implClass : implClasses) {
				cacheEnumClass(implClass);
			}
		}
	 ...
	 * 由于泛型擦除的原因，在运行时无法获取具体的泛型类型参数，因此编译器将它们擦除为Object类型。 尽管code和desc被声明为
	 * Object类型，但由于C和D的实际类型是在实现CachableEnum接口时确定的
