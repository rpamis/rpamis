...
/**
 * MethodHandle在各类开源框架中大量使用，如Mybatis等 为什么使用MethodHandle MethodHandle性能压测如何
 *
 * @author benym
 * @date: 2022/12/1 14:30
 * @link {<a href=
 * "https://www.optaplanner.org/blog/2018/01/09/JavaReflectionButMuchFaster.html">...</a>}
 * <p/>
 * 对于类外进行私有变量的访问的场景 在JDK8环境下 MethodHandle虽然单独有效，但和LambdaMetafactory.metafactory
 * BiConsumer很难结合在一起
 * @link {<a href=
 * "https://stackoverflow.com/questions/28184065/java-8-access-private-member-with-lambda">...</a>}
 * 一种更hack的结合方法是采用MethodHandle的内部方法，但由于安全性和运行原理未知，暂不使用
 * @link {<a href=
 * "https://stackoverflow.com/questions/69068124/lambdametafactory-and-private-methods">...</a>}
 * 在JDK9环境下MethodHandle提供privateLookupIn则才能和LambdaMetafactory.metafactory结合
 * <p/>
 * tip: 即使不采用LambdaMetafactory方式做到更为通用的MethodHandle,
 * 简单的静态化MethodHandle仍然是通过底层native执行方法，压测性能接近原生，远快于反射 如果使用时不采用static，则MethodHandle甚至慢于反射
 * 采用纳秒为单位，数值越小越快，从上至下，非静态化Mh、非静态化反射、静态化Mh、静态化反射 Benchmark Mode Cnt Score Error Units
 ...
 * 采用纳秒为单位，数值越小越快，从上至下，直接new set、lambda mh、无lambda的mh、反射 Benchmark Mode Cnt Score Error
 * Units MhExceptionBenchMark.MhExceptioTest.directNew avgt 10 2421.192 ± 165.195 ns/op
...
public final class MethodAccessor {
	...
	/**
	 * 参考@link{<a href=
	 * "https://stackoverflow.com/questions/53675777/how-to-instantiate-an-object-using-lambdametafactory">...</a>}
