 ...
 * 全局异常捕获，目前主要捕获spring的Validated和javax的Valid注解，和自定义异常类，全局异常类 处理顺序从上至下，被上层异常处理过的不会再被之后的异常处理
 * 适用于以Controller作为入口的接口异常捕获 包括Controller进入后调用的Service、Manager、Mapper等出现的异常
 * <p/>
...
public class ExceptionErrorHandler {
	...
	public ResponseEntity<Response<Object>> handleBindException(BindException bindException) {
		...
		String validateMessage = Objects.requireNonNull(bindException.getBindingResult().getFieldError())
			.getDefaultMessage();
		logger.warn("[BindException], 请求Id:{}, SpanId:{}, 参数校验失败:{}", trace.getTraceId(), trace.getSpanId(),
				validateMessage);
	...
	public ResponseEntity<Response<Object>> handleConsException(
		...
		String validateMessage = constraintViolations.stream()
			.map(ConstraintViolation::getMessage)
			.collect(Collectors.joining(";"));
		logger.warn("[ConstraintViolationException], 请求Id:{}, SpanId:{}, 参数校验失败:{}", trace.getTraceId(),
	...
	public ResponseEntity<Response<Object>> handleBindException(
		...
		final Trace trace = traceIdUtil.getTrace();
		String validateMessage = Objects
			.requireNonNull(methodArgumentNotValidException.getBindingResult().getFieldError())
			.getDefaultMessage();
		logger.warn("[MethodArgumentNotValidException], 请求Id:{}, SpanId:{}, 参数校验失败:{}", trace.getTraceId(),
				trace.getSpanId(), validateMessage);
		if (logger.isDebugEnabled()) {
	...
	public ResponseEntity<Response<Object>> handleBizException(BizException bizException) {
		...
		logger.warn("请求Id:{}, SpanId:{}, 业务异常:{}, 错误码:{}, 详细信息:", trace.getTraceId(), trace.getSpanId(), message,
				errCode, bizException);
		final Response<Object> failResponse = Response.fail(errCode, message);
		return new ResponseEntity<>(failResponse, HttpStatus.OK);
	}
	...
	public ResponseEntity<Response<Object>> handleBizNoStackException(BizNoStackException bizNoStackException) {
		...
		logger.warn("请求Id:{}, SpanId:{}, 业务异常(无堆栈):{}, 错误码:{}", trace.getTraceId(), trace.getSpanId(), message,
				errCode);
		if (logger.isDebugEnabled()) {
	...
	public ResponseEntity<Response<Object>> handleSysException(SysException sysException) {
		...
		logger.error("请求Id:{}, SpanId:{}, 系统异常:{}, 错误码:{}, 详细信息:", trace.getTraceId(), trace.getSpanId(), message,
				errCode, sysException);
		final Response<Object> failResponse = Response.fail(errCode, message);
	...
	public ResponseEntity<Response<Object>> handleRpasException(RpamisException rpamisException) {
		...
		logger.error("请求Id:{}, SpanId:{}, 系统内部异常:{}, 错误码:{}, 详细信息:{}", trace.getTraceId(), trace.getSpanId(), message,
				errCode, detailMessage);
		if (logger.isDebugEnabled()) {
