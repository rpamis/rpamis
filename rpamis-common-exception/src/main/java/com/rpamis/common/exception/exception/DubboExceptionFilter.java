...
/**
 * 全局dubbo异常处理，统一返回体，避免consumer端try catch手动处理rpc异常 当自定义异常处理中出现问题
...
public class DubboExceptionFilter implements Filter {
	 ...
	 * 在dubbo2.5.x以下需要配置xml，可以防止该问题 该问题只在provider端没有做统一返回体时，对consumer处理判断有影响
	...
	public Object handleValidException(ValidException exception) {
		final Trace trace = traceIdUtil.getTrace();
		logger.warn("catch dubbo validExpcetion, requestId:{}, spanId:{}, exception is:", trace.getTraceId(),
				trace.getSpanId(), exception);
		Optional<ValidException> opValid = Optional.ofNullable(exception);
		String errCode = opValid.map(AbstractException::getErrCode).orElse(ResponseCode.VALID_EXCEPTION_CODE.getCode());
	...
	public Object handleBizException(BizException exception) {
		final Trace trace = traceIdUtil.getTrace();
		logger.warn("catch dubbo bizExpcetion, requestId:{}, spanId:{}, exception is:", trace.getTraceId(),
				trace.getSpanId(), exception);
		Optional<BizException> opBiz = Optional.ofNullable(exception);
		String errCode = opBiz.map(AbstractException::getErrCode).orElse(ResponseCode.BIZ_EXCEPTION_CODE.getCode());
	...
	public Object handleBizNoStackException(BizNoStackException exception) {
		final Trace trace = traceIdUtil.getTrace();
		logger.warn("catch dubbo bizNoStackException, requestId:{}, spanId:{}, exception is:", trace.getTraceId(),
				trace.getSpanId(), exception);
	...
	public Object handleSysException(SysException exception) {
		final Trace trace = traceIdUtil.getTrace();
		logger.error("catch dubbo sysExpcetion, requestId:{}, spanId:{}, exception is:", trace.getTraceId(),
				trace.getSpanId(), exception);
		Optional<SysException> opSys = Optional.ofNullable(exception);
		String errCode = opSys.map(AbstractException::getErrCode).orElse(ResponseCode.SYS_EXCEPTION_CODE.getCode());
	...
	public Object handleRpasException(RpamisException exception) {
		final Trace trace = traceIdUtil.getTrace();
		logger.error("catch dubbo rpasExpcetion, requestId:{}, spanId:{}, exception is:", trace.getTraceId(),
				trace.getSpanId(), exception);
		Optional<RpamisException> opRpas = Optional.ofNullable(exception);
		String errCode = opRpas.map(RpamisException::getErrCode).orElse(ResponseCode.RPAS_EXCEPTION_CODE.getCode());
	...
	public Object handleException(Exception exception) {
		final Trace trace = traceIdUtil.getTrace();
		logger.error("catch dubbo unknown Expcetion, requestId:{}, spanId:{}, exception is:", trace.getTraceId(),
				trace.getSpanId(), exception);
