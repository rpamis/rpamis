...
public class RpamisBeanUtil {
	...
	public static <T> T copy(Object source, Class<T> clazz) {
		...
		try {
			...
			throw ExceptionFactory.sysException("RpamisBeanUtils copy exception:", e);
		}
		return target;
	}
	...
	public static <T> T copy(Object source, Class<T> clazz, Converter converter) {
		...
		try {
			...
			throw ExceptionFactory.sysException("RpamisBeanUtils copy with converter exception:", e);
		}
		return target;
	}
	...
	public static <S, T> PageResponse<T> toPageResponse(Page<S> page, Function<S, T> functionConverter) {
		...
		Collection<T> list = page.getRecords().stream().map(functionConverter).collect(Collectors.toList());
	 ...
	 * @return PageResponse
	 */
	public static <S, T> PageResponse<T> toPageResponse(PageResponse<S> sourcePageResponse,
			Function<S, T> functionConverter) {
		PageResponse<T> pageResponse = new PageResponse<>();
		...
		Collection<T> list = sourcePageResponse.getList().stream().map(functionConverter).collect(Collectors.toList());
		pageResponse.setList(list);
		return pageResponse;
	}
