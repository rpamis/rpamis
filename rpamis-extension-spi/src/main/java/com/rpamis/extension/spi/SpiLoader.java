...
public class SpiLoader<T> {
	 ...
	 */
	private static final ConcurrentHashMap<Class<?>, SpiLoader<?>> SPI_LOADERS = new ConcurrentHashMap<>(64);
	 ...
	 */
	private final ConcurrentHashMap<Class<?>, Object> cachedClassToSpiImpl = new ConcurrentHashMap<>(64);
	...
	private SpiLoader(Class<?> type) {
		this.type = type;
		this.pluginInjector = type == PluginInjector.class ? null
				: getSpiLoader(PluginInjector.class).getAccommodator();
	}
	...
	public void inject(Class<?> parameterType, Method method, T spiImplInstance) {
		try {
			...
			String exceptionMessage = String.format("Failed to inject via method %s of interface %s, because: %s",
	...
	private void cacheDefaultSpiName() {
		...
		if (value.length() > 0) {
			...
			if (names.length > 1) {
				throw new IllegalStateException(
						"More than 1 default spi name on spi " + type.getName() + ": " + Arrays.toString(names));
			}
			if (names.length == 1) {
	 ...
	 */
	private void loadResource(Map<String, Class<?>> spiToImplClassMap, ClassLoader classLoader, URL resourceUrl) {
		try {
				...
				while ((line = reader.readLine()) != null) {
					...
					if (line.length() > 0) {
						try {
							...
							if (line.length() > 0) {
								loadClass(spiToImplClassMap, resourceUrl, Class.forName(line, true, classLoader), name);
							}
						}
						catch (Throwable t) {
			...
			LOGGER.log(Level.SEVERE, "Exception when load extension class(interface: " + type + ", class file: "
					+ resourceUrl + ") in " + resourceUrl, e);
		}
	}

	private void loadClass(Map<String, Class<?>> spiToImplClassMap, URL resourceUrl, Class<?> clazz, String name) {
	...
	private void saveInSpiToImplClassMap(Map<String, Class<?>> spiToImplClassMap, Class<?> clazz, String name) {
